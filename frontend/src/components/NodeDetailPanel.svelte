...
>
  <div class="p-4 border-b border-border-subtle flex justify-between items-start">
    <div>
      <span
        class="inline-block px-2 py-1 font-label-md text-label-md rounded uppercase tracking-wider mb-2"
  ...
  <div class="p-4 space-y-3 overflow-y-auto">
    <div class="flex items-center justify-between">
      <span class="font-label-md text-label-md text-text-muted uppercase tracking-wider">Status</span>
    ...
    <div class="flex items-center justify-between">
      <span class="font-label-md text-label-md text-text-muted uppercase tracking-wider">Connections</span>
      <span class="font-label-md text-label-md text-on-surface">{connections} nodes</span>
    </div>

    {#if edges.length > 0}
      <div class="pt-2 border-t border-border-subtle">
        <span class="font-label-md text-label-md text-text-muted uppercase tracking-wider">Links</span>
  ...
  <div class="p-4 border-t border-border-subtle bg-surface-container-low rounded-b-xl">
    ...
    {/if}
    <button
      class="w-full mt-2 py-2 bg-transparent text-text-muted border border-border-default rounded hover:border-status-error hover:text-status-error transition-all duration-300 font-label-md text-label-md uppercase tracking-wider flex items-center justify-center gap-2"
      onclick={banish}
    >
