...
export const planner = {
  ...
  // Completion history for the habit-detail calendar. The server always returns `dates`
  // (newest first); the rest is a bonus the UI does not depend on.
  habitHistory: (id: string) =>
    get<{ dates: string[]; name?: string; cadence_type?: string; weekly_target?: number | null; streak?: Streak }>(
      `/api/planner/habits/${id}/history`,
    ),
